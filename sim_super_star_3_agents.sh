...
#PBS -A "AlgColl"
#PBS -r y
#PBS -o pbs_output/o_files/3_agents/super_stars/job_array_index_[^array_index^].OU
#PBS -e pbs_output/e_files/3_agents/super_stars/job_array_index_[^array_index^].ER

me=`basename $0`
LOGFILE=$PBS_O_WORKDIR/"pbs_output/log_files"/$PBS_JOBNAME"."$PBS_JOBID"_"$PBS_ARRAY_INDEX"_super_star.log"
...
eval 'python run_super_star_simulation.py parameter_super_star_3_agent '$PBS_ARRAY_INDEX | tee -a ${LOGFILE}
